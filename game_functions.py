...
class GameControls:
    ...
    def update_screen(screen, character, bullets, aliens, settings, buttons):
        if not settings.pause:
            ...
            GameFuncts().new_text(screen, settings, [msg, 0.1, 1])
            # Crie uma lista de retângulos que precisam ser atualizados
            pyg.display.update(updated_rects)
    
    @staticmethod
    def game_over(screen, settings, bullets, aliens, character, buttons):
        ...
        while not settings.rodando:
            for event in pyg.event.get():
                if event.type == pyg.KEYDOWN and event.key == pyg.K_r:
                    GameControls().handle_stop_restart(settings, bullets, aliens, character, True)
...
class AliensFuncts:
    ...
    def aliens_killed(settings):
        GameFuncts().new_text(settings.screen, settings, ["UFO's count: " + str(settings.count), 0.8, 20])
    ...
    @staticmethod
    def handle_game_logic(screen, character, bullets, aliens, settings, buttons):
        ...
        AliensFuncts.check_aliens_bottom(screen, character, settings, aliens, bullets, buttons)

    @staticmethod
    def check_aliens_bottom(screen, character, settings, aliens, bullets, buttons):
        ...
        for alien in aliens.sprites():
            if alien.rect.bottom >= character.rect.top - 20:
                ...
                if settings.ship_lifes < 1:
                    ...
                    GameControls().game_over(screen, settings, bullets, aliens, character, buttons)
...
class GameLoopManager:
    ...
    def run_game_loop(screen, character, bullets, settings, aliens, buttons):
        ...
        while True:
            ...
            AliensFuncts().handle_game_logic(screen, character, bullets, aliens, settings, buttons)
